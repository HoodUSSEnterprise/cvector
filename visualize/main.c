...
static int init_app(App *app)
{
    ...
    if (!font) font = TTF_OpenFont("C:/Windows/Fonts/msyh.ttc", 20);
...
static void close_app(App *app)
{
    ...
    TTF_Quit();
#endif
    if (app->stack) free_stack(app->stack);
...
static void render_text(SDL_Renderer *renderer, const char *text, int x, int y, SDL_Color color)
{
    (void)renderer; (void)text; (void)x; (void)y; (void)color;
}
#endif
